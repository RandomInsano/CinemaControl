...
/// 1 Report ID byte + voltage (u16 LE) + current (u16 LE) + temperature (i16 LE).
pub const PSU_FEATURE_REPORT_LEN: usize = 7;
/// The same three fields with no Report ID byte.
pub const PSU_INPUT_REPORT_LEN: usize = 6;
...
impl PsuTelemetry {
    pub fn from_bytes(bytes: [u8; PSU_INPUT_REPORT_LEN]) -> Self {
        Self {
            ...
            temperature_decic: i16::from_le_bytes([bytes[4], bytes[5]]),
        }
    }
}
...
impl fmt::Display for PsuTelemetry {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            f32::from(self.temperature_decic) / 10.0,
        )
    }
}
