...
to `/usr/lib/udev/rules.d/` automatically. Building from source instead,
install it to `/etc/udev/rules.d/` yourself so a logged-in user can access
the board without `sudo`:
